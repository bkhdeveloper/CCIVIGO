...
public class DateHigri {
    ...
    static String[] iMonthNames;

    public DateHigri(Context context) {
        iMonthNames = context.getResources().getStringArray(R.array.HejriMonths);
    }
    ...
    static String[] wdNames = {"Ahad", "Ithnin", "Thulatha", "Arbaa", "Khamis",
        "Jumuah", "Sabt"};

    static double[] kuwaiticalendar(boolean adjust,Calendar cal) {
        Calendar today = cal;
        int adj=0;
        if(adjust){
            adj=0;
    ...
    static String writeIslamicDate(Calendar c) {

        // This Value is used to give the correct day +- 1 day
        boolean dayTest=true;
        double[] iDate = kuwaiticalendar(dayTest, c);

        //String outputIslamicDate = wdNames[(int) iDate[4]] + ", " + (int) iDate[5] + " " + iMonthNames[(int) iDate[6]] + " " + (int) iDate[7];
